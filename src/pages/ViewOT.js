...
import {TipoOT, EtatusOT, TecnicoList, ArticuloTipoVenta, MaterialesOT, SeveCliente, GetOT, GetClientes, ListadoGen, SaveOT} from '../service/catalogos/Catalogos.service'
...
const ViewOT = () => {
    ...
    //
    const [vistaTipo, setVistaTipo] = useState(Tipo)
    const [dataTipo, setDataTipo] = useState([])

    const [vistaEstatus, setVistaEstatus] = useState(Estatus)
    const [dataEstatus, setDataEstatus] = useState([])

    const [vistaTecnico, setVistaTecnico] = useState(Tecnico)
    ...
    useEffect(() => {
        ...
        GetOT(data, Token).then(res=>{
            let data = res.OData;
            for (const i in data) {
                let newDate = ""
                if(i === "Fecha_inserta"){
                    newDate = data[i].split("T")
                }
                switch (i) {
                    case "Fecha_inserta":
                            res.OData.Fecha_inserta = newDate[0]
                        break;
                    default:
                        break;
                }
    ...
    //     const getMateriales = async (Token) => {
    //         await MaterialesOT(Token).then(res => {
    //             console.log("materiales",res)
    //             if(res.OData !== null){
    //                 setODataMaterialesOT(res.OData)
    //             }
    //         }).catch(e => {
    //             console.log(e)
    //         })
    ...
    return (
        <Menu menuTitle={`Orden de trabajo: ${Id}`}>
            <div className='row'>
                                            ...
                                            !openMoCliente ? (
                                                <div className='col-9 ps-0 mt-1'>
                                                    <Search
                                                        ...
                                                        // Value={dataOT._Cliente_nombre} _Tipo_nombre
                                                        Value={"---"} 
                            ...
                            <div  className="row">
                                <div  className="col">
                                    <Forms.InputLin
                                        ...
                                        Valueinp={dataOT._Usuario_nombre_inserta}
                                        Disabled={true}
                                        heightInp={'30px'}
                                    />
                                </div>
